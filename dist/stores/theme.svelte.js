...
function getStoredNebula() {
    if (typeof window === 'undefined')
        return false;
    return localStorage.getItem(STORAGE_KEY_NEBULA) === 'true';
}
function getStoredSolidText() {
    if (typeof window === 'undefined')
        return false; // Default to no text shadow
...
class ThemeStore {
    mode = $state(getStoredMode());
    accent = $state(getStoredAccent());
    uiScale = $state(getStoredScale());
    bgEffect = $state(getStoredBgEffect());
    bgQuality = $state(getStoredBgQuality());
    nebula = $state(getStoredNebula());
    solidText = $state(getStoredSolidText());
    colors = $derived(this.mode === 'dark' ? darkColors : lightColors);
