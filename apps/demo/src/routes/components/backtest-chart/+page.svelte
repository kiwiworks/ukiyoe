<script lang="ts">
	import { BacktestChart } from 'ukiyoe/viz';
	import { PageHeader, Text, Select, Numeric, Badge, Heading } from 'ukiyoe/core';
	...
	function generatePriceData(startPrice: number, days: number, volatility = 0.02): number[] {
	...
	function generateTrades(strategy: StrategyKey, startDate: number, initialCapital: number) {
		...
		if (strategy === 'momentum') {
			...
			tradeConfigs.forEach((config) => {
				...
				const pnlPct = isWin ? 0.015 + Math.random() * 0.035 : -(0.008 + Math.random() * 0.022);
				const exitPrice = config.side === 'BUY' ? entryPrice * (1 + pnlPct) : entryPrice * (1 - pnlPct);
			...
			tradeConfigs.forEach((config) => {
				...
				const pnlPct = isWin ? 0.008 + Math.random() * 0.018 : -(0.006 + Math.random() * 0.014);
				const exitPrice = config.side === 'BUY' ? entryPrice * (1 + pnlPct) : entryPrice * (1 - pnlPct);
				const positionValue = equity * positionSize;
				const pnl = positionValue * pnlPct;
				equity += pnl;
			...
			tradeConfigs.forEach((config) => {
				...
				const pnlPct = isWin ? 0.025 + Math.random() * 0.055 : -(0.012 + Math.random() * 0.018);
				const exitPrice = config.side === 'BUY' ? entryPrice * (1 + pnlPct) : entryPrice * (1 - pnlPct);
				const positionValue = equity * positionSize;
				const pnl = positionValue * pnlPct;
				equity += pnl;
	...
	function calculateStats(trades: typeof momentumData.trades, initialCapital: number, finalEquity: number) {
		...
		return { totalTrades: trades.length, winRate, totalPnl, totalReturn, profitFactor, maxDrawdown };
	}
			...
			<div class="p-xl border-b border-border-subtle">
				<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-xl">
					<div class="flex items-center gap-lg">
						<span class="text-sm text-text-muted font-medium">Strategy:</span>
						<div class="w-48">
							<Select bind:value={selectedStrategy} options={strategyOptions} size="sm" />
						</div>
					</div>
