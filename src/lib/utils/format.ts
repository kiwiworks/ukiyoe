...
export function formatCompact(value: number, opts?: FormatOptions): string {
	...
	const abs = Math.abs(value);
	if (abs >= 1e9) return `${(value / 1e9).toFixed(decimals ?? 2)}B`;
	if (abs >= 1e6) return `${(value / 1e6).toFixed(decimals ?? 2)}M`;
	if (abs >= 1e3) return `${(value / 1e3).toFixed(decimals ?? 1)}K`;
	return value.toFixed(decimals ?? 0);
}

/** Format microseconds as human-readable duration: 500µs, 1.2ms, 2.5s */
export function formatDuration(microseconds: number, opts?: FormatOptions): string {
	const decimals = opts?.decimals;
	const abs = Math.abs(microseconds);
	if (abs < 1000) return `${abs.toFixed(decimals ?? 0)}µs`;
	if (abs < 1_000_000) return `${(abs / 1000).toFixed(decimals ?? 1)}ms`;
...
export function formatBytes(bytes: number, opts?: FormatOptions): string {
	...
	if (abs < 1024) return `${abs.toFixed(decimals ?? 0)}B`;
	if (abs < 1024 * 1024) return `${(abs / 1024).toFixed(decimals ?? 1)}KB`;
	if (abs < 1024 * 1024 * 1024) return `${(abs / (1024 * 1024)).toFixed(decimals ?? 2)}MB`;
	return `${(abs / (1024 * 1024 * 1024)).toFixed(decimals ?? 2)}GB`;
}

/** Format as currency using Intl: $1,234.56 */
export function formatCurrency(value: number, currency: string, opts?: FormatOptions): string {
	const locale = opts?.locale ?? 'en-US';
	const decimals = opts?.decimals ?? 2;
	return getFormatter(locale, {
		style: 'currency',
...
export function formatBps(value: number, opts?: FormatOptions): string {
...
export function formatTokenRate(value: number, opts?: FormatOptions): string {
	const decimals = opts?.decimals ?? 1;
